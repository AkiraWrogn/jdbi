...
This `QualifiedType` still represents the `String` _type_, but _qualified_ with `@Json` annotation.
...
[WARNING]
Qualifiers being annotations does *not* mean their features are guaranteed to work
when placed on source classes.
